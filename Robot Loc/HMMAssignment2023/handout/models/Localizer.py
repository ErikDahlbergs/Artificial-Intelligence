...
class Localizer:
    def __init__(self, sm : StateModel):
        ...
        #self.__rs = RobotSimAndFilter(self.__sm, self.__tm, self.__om)
    ...
    def initialise(self):
        ...
        self.__sense = None
        self.__fVec = np.ones(self.__sm.get_num_of_states()) / (self.__sm.get_num_of_states()) #Always 1?
    ...
    # add your simulator and filter here, for example    
        ...
        self.__HMM = RobotSimAndFilter.HMMFilter(self.__tm, self.__om, self.__sm)
    #
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)):
        ...
        self.__trueState = self.__rs.move(self.__trueState) #Move the robot and return a true state
        tsX, tsY, tsH = self.__sm.state_to_pose(self.__trueState) #Because sensor_reading ended up taking pose as input arguments, true state is converted to pose
        self.__sense = self.__rs.sensor_reading(tsX, tsY, tsH) #Calulcate a sensor reading based on true state. 

        self.__fVec, self.__estimate = self.__HMM.filtering(self.__sense, self.__fVec) #Filter the current sensor reading with previous f_vec distribution
        ...
        # rows, columns, __ = self.__sm.get_grid_dimensions()
        # eX, eY = random.choice(range(rows)), random.choice(range(columns))
        
        # this should be updated to spit out the actual error for this step
        error = abs(tsX-eX) + abs(tsY-eY)  #Norm distance            
