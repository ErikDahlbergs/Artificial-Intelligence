...
class TransitionModel:
	def __init__(self, stateModel):
		...
		self.__rows, self.__cols, self.__head = self.__sm.get_grid_dimensions() #Gets dimensions from state model

		self.__dim = self.__rows * self.__cols * self.__head #Compile dimensions into own variable dim. This corresponds to number dimensions in statemodel

		self.__matrix = np.zeros(shape=(self.__dim, self.__dim), dtype=float) #Creates blank grid
		for i in range(self.__dim): #Iterates through number of states
			x, y, h = self.__sm.state_to_pose(i) #Derives position for every state i
			for j in range(self.__dim): #Iterates for available coming states
				nx, ny, nh = self.__sm.state_to_pose(j) #Derives position for new states
				...
				if abs(x - nx) + abs(y - ny) == 1 and \
					...
					if nh == h: #Comes from definition in assignment description
						self.__matrix[i, j] = 0.7

					else: # entry where new and old heading are different, i.e., distributing probabilities for the "rest"
						if x != 0 and x != self.__rows - 1 and y != 0 and y != self.__cols - 1: #Checks that state is not by a wall
							self.__matrix[i, j] = 0.1 #Borde inte detta vara 0.3?
						...
						# Facing a wall, not in a corner    
							...
							#1st = towards top row, 2nd = towards right wall, 3rd = towards bottom row, 4th towards left wall
